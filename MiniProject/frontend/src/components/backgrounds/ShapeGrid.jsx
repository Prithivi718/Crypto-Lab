...
const ShapeGrid = ({
    ...
    useEffect(() => {
        ...
        // Draw small '+' marker at grid intersections as specified in plan.txt
        const drawPlusMarker = (x, y, size = 3) => {
            ctx.beginPath();
            ctx.moveTo(x - size, y);
            ctx.lineTo(x + size, y);
            ctx.moveTo(x, y - size);
        ...
        const drawGrid = () => {
            ...
            if (isHex) {
                ...
                // Default Square Grid with + Markers
                const offsetX = ((gridOffset.current.x % squareSize) + squareSize) % squareSize;
                const offsetY = ((gridOffset.current.y % squareSize) + squareSize) % squareSize;
                ...
                for (let col = -2; col < cols; col++) {
                    for (let row = -2; row < rows; row++) {
                        ...
                        // Draw technical + marker at top-left corner of each cell
                        drawPlusMarker(sx, sy);
                    }
                }
            }
            ...
            gradient.addColorStop(0, 'rgba(17, 18, 15, 0)');
            gradient.addColorStop(1, 'rgba(17, 18, 15, 0.7)');
