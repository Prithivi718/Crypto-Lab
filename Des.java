...
public class Des {
    ...
    static long bitsToLong(int[] bits) {
        long value = 0;
        for (int bit : bits) value = (value << 1) | bit;
        return value;
    }
    ...
    static long textToLong(String text) {
        ...
        for (int i = 0; i < 8; i++) bytes[i] = (i < src.length) ? src[i] : 0x20;
        long val = 0;
    ...
    static String longToText(long val) {
        ...
        for (int i = 7; i >= 0; i--) sb.append((char) ((val >> (i * 8)) & 0xFF));
    ...
    static String bitsToGroupedString(int[] bits, int group) {
        ...
        for (int i = 0; i < bits.length; i++) {
            sb.append(bits[i]);
            if ((i + 1) % group == 0 && i != bits.length - 1) sb.append(' ');
        }
        return sb.toString();
    }
    ...
    static int[] substitute(int[] bits) {
        ...
        for (int s = 0; s < 8; s++) {
            ...
            for (int b = 3; b >= 0; b--) out[s * 4 + (3 - b)] = (v >> b) & 1;
        }
        return out;
    }
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < 16; i++) decryptKeys[i] = roundKeys[15 - i];
